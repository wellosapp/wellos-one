...
// React 19's useActionState. Next 14 runs on React 18 — see ClientTagForm
...
import { addClientNoteAction, type ActionState } from '../_actions';
...
export function NotesTab({ clientId, appointmentId, notes }: NotesTabProps) {
  const router = useRouter();
  const formRef = useRef<HTMLFormElement>(null);
  const action = addClientNoteAction.bind(null, clientId, appointmentId);
  const [state, formAction] = useFormState<ActionState, FormData>(
    action,
    INITIAL,
  );

  // After a successful submit, reset the form and pull fresh notes via
  // router.refresh() so the new note shows up in the list.
  useEffect(() => {
    if (state.ok) {
  ...
  return (
    <div className="flex flex-col gap-s5">
      <section className="flex flex-col gap-s3">
        <h3 className="t-display-sm text-ink">Linked notes</h3>
        <LinkedNotesList
          notes={notes}
          emptyLabel="No notes for this appointment yet."
        />
      </section>

      <section className="flex flex-col gap-s3 border-t border-surface-3 pt-s5">
        <h3 className="t-display-sm text-ink">Add a note</h3>
        ...
        >
          <div className="grid grid-cols-1 gap-s3 sm:grid-cols-2">
            ...
            >
              <Select name="category" defaultValue="session">
                {CATEGORIES.map((c) => (
          ...
          >
            <Textarea
              ...
              required
              maxLength={8000}
              placeholder="What should the next visit know?"
            />
          </FormField>
