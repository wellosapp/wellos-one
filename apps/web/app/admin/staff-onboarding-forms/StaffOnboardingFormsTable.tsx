...
import {
  formatStaffOnboardingFormDate,
  staffOnboardingFormDescriptionLine,
  staffOnboardingStatusLabel,
  staffOnboardingStatusTone,
...
export function StaffOnboardingFormsTable({ definitions }: Props) {
              ...
              >
                <td className="px-s4 py-s4 align-top">
                  <div className="flex flex-col gap-s1">
                    ...
                    <span className="max-w-md t-caption leading-snug text-ink-soft">
                      {staffOnboardingFormDescriptionLine(d.schema)}
                    </span>
                  </div>
                </td>
                <td className="px-s4 py-s4 align-top">
                  <Badge tone={staffOnboardingStatusTone(d.status)}>
                    {staffOnboardingStatusLabel(d.status)}
                  </Badge>
                </td>
                <td className="px-s4 py-s4 align-top">
                ...
                <td className="px-s4 py-s4 align-top">
                  <span className="t-body-sm text-ink-soft whitespace-nowrap">
                    {formatStaffOnboardingFormDate(d.updatedAt)}
                  </span>
                </td>
                <td className="px-s4 py-s4 align-top">
                  <div className="flex flex-col items-end gap-s2 sm:flex-row sm:justify-end sm:gap-s3">
                    <Link
                      href={`/admin/staff-onboarding-forms/${d.id}`}
                      className={`${editButtonClassName} no-underline`}
                    >
                      Edit
                    </Link>
                    <Link
                      href={`/admin/staff-onboarding-forms?groupId=${encodeURIComponent(d.groupId)}`}
