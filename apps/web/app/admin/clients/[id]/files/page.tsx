...
// /admin/clients/:id/files — server-rendered files tab. Matches the visual
// fidelity established by the Notes / Visits / Book tabs. URL drives state:
...
export default async function ClientFilesTabPage({
  ...
  function makeHref(next: {
    ...
    return (`/admin/clients/${id}/files${qs ? `?${qs}` : ''}`) as Route;
  }
    ...
    >
      ...
      >
        ...
        <h2 className="mt-s2 font-display text-[22px] leading-tight text-ink">
          Files for {client.firstName}.
        </h2>
        <p className="mt-s2 max-w-2xl t-body-md leading-relaxed text-ink-3">
          Photos, documents, and uploads connected to this client and their
          appointments.
        </p>
      </header>
      ...
      <div className="flex flex-col gap-s4 p-s6 lg:p-s8">
        ...
        {uploadOpen && (
          <FilesDropzone clientId={id} closeHref={closeUploadHref} />
        )}
