...
import type { StaffOnboardingFormStatusFilter } from './staff-onboarding-forms-dashboard-helpers';

function isStatusFilter(v: string): v is StaffOnboardingFormStatusFilter {
...
export function StaffOnboardingFormsFilters({ status, q, groupId }: Props) {
  const clearHref = '/admin/staff-onboarding-forms';
    ...
    <Card padding="sm">
      <form method="get" className="flex flex-wrap items-center gap-s3">
        <Input
          ...
          autoComplete="off"
          aria-label="Search staff onboarding forms by title"
        />
        <Select
          name="status"
