...
import {
  publishStaffOnboardingFormDefinitionAction,
  saveStaffOnboardingFormDefinitionAction,
  type StaffOnboardingFormEditorState,
} from './actions';

const initialSaveState: StaffOnboardingFormEditorState = { ok: false };
...
export function StaffOnboardingFormEditor({
  definition,
}: {
  definition: StaffOnboardingFormDefinitionDto;
}) {
  const [saveState, saveAction] = useFormState<StaffOnboardingFormEditorState, FormData>(
    saveStaffOnboardingFormDefinitionAction,
    initialSaveState,
  );
    ...
    <div className="space-y-s6">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-s4">
          <div>
            <span className="t-eyebrow text-accent">Staff onboarding form</span>
          ...
          {!readOnly ? (
            <Button
              ...
              onClick={() => {
                setPubMessage(null);
                startPub(async () => {
                  const r = await publishStaffOnboardingFormDefinitionAction(
                    definition.id,
                  );
